    ...
    public class RT_EFLUENTEController : ControllerBase
    {
        ...
        public IActionResult GetById(int id)
        {
            try
            {
                ...
                if (attachment == null)
                {
                    return NotFound(new { mensagem = "Anexo n�o encontrado" });
                }
        ...
        public async Task<IActionResult> UploadAttachment([FromForm] string relObjectId, IFormFile file)
        {
            try
            {
                ...
                {
                    return BadRequest(new { mensagem = "ID do efluente � obrigat�rio" });
                }

                if (file == null || file.Length == 0)
                {
                    return BadRequest(new { mensagem = "Arquivo n�o fornecido" });
                }

                // Validate file size
                if (file.Length > MaxFileSize)
                {
                    return BadRequest(new { mensagem = $"Arquivo muito grande. Tamanho m�ximo: {MaxFileSize / (1024 * 1024)} MB" });
                }
                ...
                {
                    return BadRequest(new { mensagem = "Tipo de arquivo n�o permitido" });
                }
                ...
                // Create attachment
                var nextId = _attachmentRepository.GetNextId();
        ...
        public IActionResult DownloadAttachment(int id)
        {
            try
            {
                ...
                {
                    return BadRequest(new { mensagem = "Arquivo n�o cont�m dados" });
                }
